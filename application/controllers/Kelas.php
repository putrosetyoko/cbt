...
class Kelas extends CI_Controller
{
    ...
    public function add()
    {
        $data = [
            ...
            // 'jurusan' => $this->master->getAllJurusan() // Dihapus karena kelas tidak lagi berelasi langsung dengan jurusan
        ];
        $this->load->view('_templates/dashboard/_header.php', $data);
    ...
    public function edit()
    {
        ...
        if (!$chk) {
            ...
            $data = [
                ...
                // 'jurusan' => $this->master->getAllJurusan(), // Dihapus
                'kelas'     => $kelas
            ];
            $this->load->view('_templates/dashboard/_header.php', $data);
    ...
    public function save()
    {
        ...
        for ($i = 1; $i <= $rows; $i++) {
            ...
            // $jurusan_id   = 'jurusan_id[' . $i . ']'; // Dihapus
            $this->form_validation->set_rules($nama_kelas, 'Kelas', 'required');
            // $this->form_validation->set_rules($jurusan_id, 'Jurusan', 'required'); // Dihapus
            $this->form_validation->set_message('required', '{field} Wajib diisi');

            if ($this->form_validation->run() === FALSE) {
                $error[] = [
                    $nama_kelas   => form_error($nama_kelas),
                    // $jurusan_id   => form_error($jurusan_id), // Dihapus
                ];
                $status = FALSE;
            } else {
                if ($mode == 'add') {
                    $insert[] = [
                        ...
                        // 'jurusan_id'  => $this->input->post($jurusan_id, true) // Dihapus
                    ];
                } else if ($mode == 'edit') {
                    $update[] = array(
    ...
    // public function kelas_by_jurusan($id) // Dihapus karena tidak lagi relevan
    // {
    ...
    public function do_import()
    {
        ...
        $data = [];
        foreach ($input as $d) {
            $data[] = ['nama_kelas' => $d->kelas]; // Hanya nama_kelas
        }
