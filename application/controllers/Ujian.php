...
class Ujian extends CI_Controller {
  ...
  public $siswa, $user, $guru; // Added $guru property, changed $mhs to $siswa
  ...
  public function __construct(){
    ...
    $this->siswa = $this->ujian->getIdSiswa($this->user->username); // Changed getIdMahasiswa to getIdSiswa
    $this->guru = $this->ujian->getIdGuru($this->user->username); // Added guru property
  }

  public function akses_guru() // Renamed akses_dosen to akses_guru
  {
    if ( !$this->ion_auth->in_group('guru') ){ // Changed 'dosen' to 'guru'
      show_error('Halaman ini khusus untuk guru untuk membuat Test Online, <a href="'.base_url('dashboard').'">Kembali ke menu awal</a>', 403, 'Akses Terlarang'); // Updated text
    }
  }
  ...
  {
        $this->akses_guru(); // Changed akses_dosen to akses_guru
  ...
  public function master()
  {
      ...
      'judul' => 'Ujian',
      'subjudul'=> 'Data Ujian',
      'guru' => $this->ujian->getIdGuru($user->username), // Changed dosen to guru and getIdDosen to getIdGuru
    ];
  ...
  public function add()
  {
    ...
    $user = $this->ion_auth->user()->row();
      ...
      'mapel'   => $this->soal->getMapelGuru($user->username), // Changed matkul to mapel and getMatkulDosen to getMapelGuru
      'guru'    => $this->ujian->getIdGuru($user->username), // Changed dosen to guru and getIdDosen to getIdGuru
    ];
  ...
  public function validasi()
  {
    ...
    $user   = $this->ion_auth->user()->row();
    $guru   = $this->ujian->getIdGuru($user->username); // Changed dosen to guru and getIdDosen to getIdGuru
    $jml    = $this->ujian->getJumlahSoal($guru->id_guru)->jml_soal; // Changed dosen to guru and id_dosen to id_guru
  ...
  public function save()
  {
    ...
    $guru_id      = $this->input->post('guru_id', true); // Changed dosen_id to guru_id
    $mapel_id     = $this->input->post('mapel_id', true); // Changed matkul_id to mapel_id
    ...
    if( $this->form_validation->run() === FALSE ){
      ...
      if($method === 'add'){
        $input['guru_id']   = $guru_id; // Changed dosen_id to guru_id
        $input['mapel_id'] = $mapel_id; // Changed matkul_id to mapel_id
        $input['token']   = $token;
  ...
  public function list_json()
  {
    ...
    $list = $this->ujian->getListUjian($this->siswa->id_siswa, $this->siswa->kelas_id); // Changed mhs to siswa and id_mahasiswa to id_siswa
  ...
  public function list()
  {
    ...
    $data = [
      ...
      'judul'   => 'Ujian',
      'subjudul'    => 'List Ujian',
      'siswa'     => $this->ujian->getIdSiswa($user->username), // Changed mhs to siswa and getIdMahasiswa to getIdSiswa
    ];
    $this->load->view('_templates/dashboard/_header.php', $data);
  ...
  public function index()
  {
    ...
    $siswa    = $this->siswa; // Changed mhs to siswa
    $h_ujian  = $this->ujian->HslUjian($id, $siswa->id_siswa); // Changed mhs to siswa and id_mahasiswa to id_siswa
    ...
    if ($cek_sudah_ikut < 1) {
      ...
      $input = [
        'ujian_id'    => $id,
        'siswa_id'    => $siswa->id_siswa, // Changed mahasiswa_id to siswa_id and mhs to siswa
